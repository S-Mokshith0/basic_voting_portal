			...
			<table border="0%">
				...
				<?php while ($row = mysqli_fetch_array($results)) { ?>
					<tr>
						...
						<td><?php echo $row['candi_photo']; ?></td>
						<td><?php echo $row['candi_symbol']; ?></td>
						<td><?php echo $row['contest_villa']; ?></td>
						
						<td>
							<button name="vote" type="submit" value="<?php echo $row['candi_id'] ; ?>" class="btn btn-success">Cast Vote</button>
						</td>
					</tr>
				<?php } ?>
			</table>
...
<?php
			if(isset($_POST['vote']))
			{
				...
				$results = mysqli_query($con, "SELECT * FROM voter where username='$user'");
				 ...
				 $date =date('Y/m/d');
				$query1=" INSERT INTO voting_details (voter_id, candidate_id,date_of_vote,status) VALUES ('$voterid','$vote','$date','unupdated') ;";
						$query_run1=mysqli_query($con,$query1);
						if($query_run1)
						{  
							
							$query2=" update candidate set no_of_votes=no_of_votes+1 where candi_id='$vote';";
